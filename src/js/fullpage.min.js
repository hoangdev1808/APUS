...
function parallaxScroll(e) {
	var a = 0;
	(a = isFirefox ? -120 * e.detail : isIe ? -e.deltaY : e.wheelDelta),
		1 != ticking &&
			(a <= -scrollSensitivitySetting &&
				((ticking = !0),
...
function nextItem() {
	...
	var e = pageSlide.eq(currentSlideNumber - 1),
		a = pageSlide.eq(currentSlideNumber);
...
function srcollHeaderChild() {
	if ($(".full-page") >= 1) {
		$(".tuyen-dung-ct-page header, .news-detail-page header"),
			$(
...
function createPagination() {
	...
	for (var e = 0; e < totalSlideNumber; e++)
		0 == e
	...
	$("#nav-pagination ul li").on("click", function () {
		...
		if (
			...
			for (var a = currentSlideNumber; a < totalSlideNumber; a++)
				$(
					".full-page section:nth-child(" +
						(currentSlideNumber + 1) +
						")"
			...
			for (var t = 0; t < currentSlideNumber; t++)
				$(".full-page section:nth-child(" + (t + 1) + ")").addClass(
					"down-scroll"
				);
		}
...
function fakeLogo() {
	var e = $("header .logo-wrapper").html();
...
function fakeLanguage() {
	var e = $("header .language-wrapper").html();
	$("#fake-header .menu-fake-header").append(e),
		$("#fake-header-open").on("click", function () {
			$("#fake-header").addClass("active");
		}),
...
function backToTop() {
	$(window).outerWidth() > 1025
		? $(".back-to-top").on("click", function () {
				...
				if (
					...
					(currentSlideNumber = e),
						...
						$("#nav-pagination ul li").removeClass("active"),
						$(
							"#nav-pagination ul li[data-section=".concat(
								currentSlideNumber + 1,
								"]"
							)
						).addClass("active");
					...
					for (var t = 0; t < currentSlideNumber; t++)
						$(
							".page-wrap section:nth-child(" + (t + 1) + ")"
		...
		$(".news-detail-page").scroll(function (e) {
			var a = $(".news-detail-page").scrollTop();
			a > 5
				? ($("header").addClass("active"),
				  $("#fake-header-open").addClass("active"))
				: ($("header").removeClass("active"),
